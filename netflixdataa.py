...
print(f"Contries's contribution to netflix: {countries_and_production.index[0]} has the most number of movies and shows on Netflix. This might be because it's an English-speaking country and has a big film industry.")
...
ax[0][1].legend()
print(f"Types of content: {Types_of_production.index[1]}s are more than {Types_of_production.index[0]}s on Netflix. Maybe people prefer watching movies more, or they are easier to produce.")
...
max_year = Release_years[Release_years == Release_years.values.max()].index[0]
...
ax[1][1].pie(High_ratings.values, labels=High_ratings.index , autopct='%1.1f%%',colors=['#ff9999','#66b3ff','#99ff99','#ffcc99','#c2c2f0'])
ax[1][1].set_title('Top 5 ratings distribution', fontsize=10)
top_rating = High_ratings.idxmax()
